...
class Metric:
    ...
    def align(self, gt: TimeSeries, pred: TimeSeries) -> Tuple[TimeSeries, TimeSeries]:
        ...
        horizon = pred.offsets.max() // pd.Timedelta(1, pred.freq)
        logger.debug(
        ...
        ts_gt_shifted = gt.shift_to_repeat_to_multihorizon(horizon=horizon)
        logger.debug(f"\ngt_shifted:\n{ts_gt_shifted}")
        common_index = ts_gt_shifted.data.index.intersection(pred.data.index)
        logger.debug(
            f"Common index determined to be\n{common_index.to_frame(index=False)}"
        )
        ts_gt_shifted.data = ts_gt_shifted.data.loc[common_index]
        old_pred_len = len(pred)
        pred.data = pred.data.loc[common_index]
        dropped_gt_steps = len(gt) - len(ts_gt_shifted)
        dropped_pred_steps = old_pred_len - len(pred)
        ...
        return ts_gt_shifted, pred

    def __call__(self, gt: TimeSeries, pred: TimeSeries) -> Any:
