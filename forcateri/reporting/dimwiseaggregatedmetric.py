...
class DimwiseAggregatedMetric(Metric):
    ...
    def __init__(self, reductions: OrderedDict[List[str], Callable]):
    ...
    def compute_metric(self,
        ...
        group_by = sorted(
            list(
                {DimwiseAggregatedMetric.OFFSET, DimwiseAggregatedMetric.TIME_STEP, DimwiseAggregatedMetric.FEATURE, DimwiseAggregatedMetric.REPRESENTATION}
                - set(
                    [*axes, DimwiseAggregatedMetric.REPRESENTATION]
        ...
        if len(group_by) == 0:
            ...
            return pd.DataFrame(
                ...
                columns=DimwiseAggregatedMetric.get_level_values(flat_pred, DimwiseAggregatedMetric.REPRESENTATION),
            )
            ...
            reduced_index = pd.MultiIndex.from_product(
                [DimwiseAggregatedMetric.get_level_values(ts_gt.data, axis).unique() for axis in group_by]
            )
